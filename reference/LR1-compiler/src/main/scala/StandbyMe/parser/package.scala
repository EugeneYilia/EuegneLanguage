...
package object parser {
  ...
  def reduce(production: Production, right: Vector[Node]): Node = {
    production match {
      ...
      case (FUNCTIONS, Vector(FUNCTION, FUNCTIONS)) => FunctionsNode(right(0).asInstanceOf[FunctionsNode].list :+ right(1).asInstanceOf[FunctionNode])
...
//    val actionList = action.toList
//    val actionTreeMap = mutable.TreeMap[]()
//    println("sortGotoMap")
//    val gotoList = goto.toList
//    mutable.TreeMap(gotoList: _*).foreach(println)
